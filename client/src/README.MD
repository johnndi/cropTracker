...
JWTs are stored in httpOnly cookies rather than localStorage or returned as Bearer tokens. This prevents XSS attacks from stealing the token via JavaScript. The cookie is also flagged `sameSite: strict` to block CSRF. The trade-off is that Postman and native mobile clients need to handle cookies explicitly — a minor inconvenience for significantly better security.
